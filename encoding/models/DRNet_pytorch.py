...
__all__ = ['DRN', 'DRN_v2', 'DRN_v3', 'resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet101_v3',
...
class DRN_v2(nn.Module):
    def __init__(self, block, layers, num_classes=1000, channels=(64, 128, 256, 512), batchnorm=nn.BatchNorm2d,
                 pool_size=28, dilated=True, deep_base=True, multi_grid=False, multi_dilation=None, output_stride=8, high_rates=[2, 4]):
        ...
        if dilated:
            if multi_grid:
                self.layer3 = self._make_layer(block, channels[2], layers[2], stride=1, dilation=high_rates[0], batchnorm=batchnorm)
                self.layer4 = self._make_layer(block, channels[3], layers[3], stride=1, dilation=high_rates[1], batchnorm=batchnorm,
                                               multi_grid=multi_grid, multi_dilation=multi_dilation)
            else:
                self.layer3 = self._make_layer(block, channels[2], layers[2], stride=1, dilation=high_rates[0], batchnorm=batchnorm)
                self.layer4 = self._make_layer(block, channels[3], layers[3], stride=1, dilation=high_rates[1], batchnorm=batchnorm)
...
class DRN_v3(nn.Module):
    def __init__(self, block, layers, num_classes=1000, channels=(64, 128, 256, 512), batchnorm=nn.BatchNorm2d,
                 pool_size=28, dilated=True, deep_base=True, multi_grid=False, multi_dilation=None, output_stride=8, high_rates=[2, 4]):
        super(DRN_v3, self).__init__()
        if output_stride == 8:
            layer1_stride = 1
            layer2_stride = 2
        elif output_stride == 16:
    ...
    def _make_layer(self, block, planes, n_blocks, stride=1, dilation=1, batchnorm=None, multi_grid=False,
        ...
        if multi_grid == False:
            if dilation == 1 or dilation == 2:
                layers.append(
                    block(self.inplanes, planes, stride, downsample, dilation=(dilation, dilation), batchnorm=batchnorm))
            elif dilation == 4:
                layers.append(
                    block(self.inplanes, planes, stride, downsample, dilation=(dilation, dilation), batchnorm=batchnorm))
            elif dilation == 8:
...
def resnet18(pretrained=False, root='./pretrain_models', **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DRN_v2(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18'], model_dir=root))
        print('loaded pretrained resnet18 !!!')
    return model


def resnet34(pretrained=False, root='./pretrain_models', **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DRN_v2(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34'], model_dir=root))
        print('loaded pretrained resnet34 !!!')
    return model


def resnet50(pretrained=False, root='./pretrain_models', pretrained_file=None, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DRN_v2(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        '''
        if deep_base:
...
def resnet101(pretrained=False, root='./pretrain_models',pretrained_file=None, **kwargs):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DRN_v2(Bottleneck, [3, 4, 23, 3], **kwargs)
    print('******************************')
...
def resnet101_v3(pretrained=False, root='./pretrain_models',pretrained_file=None, **kwargs):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DRN_v3(Bottleneck, [3, 4, 23, 3], **kwargs)
    print('******************************')
    # Remove the following lines of comments
    # if u want to train from a pretrained model
    if pretrained:
...
def resnet152(pretrained=False, root='./pretrained_models', **kwargs):
    """Constructs a ResNet-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = DRN_v2(Bottleneck, [3, 8, 36, 3], **kwargs)
    if pretrained:
